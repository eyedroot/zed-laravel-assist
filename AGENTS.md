...
- Current container-resolution providers cover the `app(...)`/`resolve(...)` helpers and the `make`/`makeWith`/`build`/`get`/`factory`/`bound`/`has` methods on the `App` facade, `app()`, `$this->app`, and `Container::getInstance()`, offering binding-name completions, class-name completions, resolved-instance member completions/type hints, and go-to-definition (including members of the resolved type and its bound concrete). The entry-point set is defined once in `server/src/containerResolution.ts` so completion, definition, hover, and references stay in sync; the protected `resolve()` container method and non-class-arg methods (`call`/`wrap`/`tagged`/`bind*`/`singleton*`) are deliberately excluded.
...
- Current class navigation includes a project-wide PHP class/interface/trait/enum hierarchy index (`extends`/`implements` resolved to FQCNs) that powers `textDocument/implementation` (Go to Implementation) for transitive subclasses and implementers. It is on by default and can be disabled through `implementations.enabled` in the server's `initializationOptions`, which Zed forwards from `lsp.laravel-assist.initialization_options`.

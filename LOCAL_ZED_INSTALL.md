...
`code_lens`는 Zed가 CodeLens UI를 요청하고 표시할지 결정하는 전역 설정입니다. 이 설정만으로 usage가 자동 생성되지는 않고, Laravel Assist가 `textDocument/codeLens` 응답으로 PHP class/public method usage 데이터를 제공해야 화면에 표시됩니다.
...
- PHP class/public method 위의 `1 usage`/`N usages` CodeLens 표시와 클릭 시 reference 위치 열기 (`code_lens` 설정 필요)

...
2. `npm run build` → 루트에서 `cargo build --release --target wasm32-wasip2` → `cp target/wasm32-wasip2/release/laravel_assist_zed_extension.wasm extension.wasm`
   ...
   - 색인 스키마가 바뀌면 `projectIndex.ts`의 `LARAVEL_INDEX_CACHE_VERSION`을 반드시 올린다 (현재 35).
...
- 2026-07-07 | Claude (Fable 5) | Livewire 컴포넌트 인텔리전스 완료: 색인(v2/v3 디렉터리, 라이프사이클 메소드 제외한 공개 액션·프로퍼티 추출), 태그/디렉티브 완성·hover·이동·진단·quick fix, 태그 매개변수 완성, wire: 바인딩 완성·이동. 캐시 버전 34→35. 전체 테스트 201개 통과 + 임시 프로젝트 디스크 스캔 통합 검증. | 다음: 필요 시 서버 스크립트 신규 버전 릴리스로 배포 반영.
...
- Livewire 컴포넌트 인텔리전스(캐시 버전 35): `app/Livewire`(v3)·`app/Http/Livewire`(v2) 색인, `<livewire:...>`/`@livewire(...)` 완성·hover·이동·진단·quick fix, 태그 매개변수(공개 프로퍼티 kebab-case) 완성, 컴포넌트 자신의 뷰(`resources/views/livewire/**`) 안에서 `wire:model`(프로퍼티)/`wire:click` 등(액션 메소드) 완성과 클래스 파일 이동
- 보류: 코드 생성 명령(파일 스캐폴딩은 범위 밖)
